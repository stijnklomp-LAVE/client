...
Run audits from inside the project's dev container (see the development skill) — never install audit tools globally on the host:

```bash
# Lighthouse accessibility audit
docker compose --profile dev run --rm dev npx lighthouse http://localhost:3000 --only-categories=accessibility

# axe-core (via npx, no global install)
docker compose --profile dev run --rm dev npx @axe-core/cli http://localhost:3000
```
