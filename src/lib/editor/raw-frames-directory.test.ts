...
describe("saveFrame", () => {
	test("creates project and recording directories and writes frame with atomic rename", async () => {
		const rootDir = createMockDirectoryHandle("root")
		const { saveFrame } = await import("./raw-frames-directory")
		...
		expect(result).toBe("frame_000000.jpg")
		expect(
			(
	...
	test("writes to .tmp file then renames to final name", async () => {
		...
		expect(result).toBe("frame_000000.png")

		const tmpHandle = (await framesDir.getFileHandle("frame_000000.tmp", {
			create: true,
		})) as unknown as MockFileHandle
		const writable = await tmpHandle.createWritable()
		const mockWritable = writable as unknown as {
			write: { mock: { calls: unknown[][] } }
			close: { mock: { calls: unknown[][] } }
		}
		expect(mockWritable.write.mock.calls[0]?.[0]).toEqual(blob)
		expect(mockWritable.close.mock.calls.length).toBeGreaterThan(0)
		expect(
			(tmpHandle.move as unknown as { mock: { calls: unknown[][] } }).mock
				.calls[0],
		).toEqual(["frame_000000.png"])
	})

	test("retries on failure up to 3 times and eventually succeeds", async () => {
		const framesDir = createMockDirectoryHandle("frames")
		const recordingDir = createMockDirectoryHandle("recording456", {
			frames: framesDir,
		})
		...
		let attempts = 0
		framesDir.getFileHandle = mock(
			(_fileName: string, _opts?: { create?: boolean }) => {
				attempts++
				...
				return Promise.resolve(createMockFileHandle("frame_000000.tmp"))
			},
		)
